...
class _Channel:
    ...
    def __init__(self, energy_grid, cross_section_grid, e_range, energy_loss_eV, color = "#00000000"):
        ...
        self.color_uint = np.int32(int(color.lstrip("#"), 16))
...
_O2_CHANNELS = [
    # B3 state <- ground (X3)  B³Σu⁻ (labelled C3sigma_u- in Physics of the Aurora and Airglow)
    _Channel([9.74460,15.1278,20.1081,30.4191,51.0347,92.5320,147.414,267.280,404.335,551.535],
             ...
             (10,550), 6.120),      #invisible, UV
    
    # a1 state 	a¹Δg <- ground (X3)
    ...
    _Channel([1.73211,2.00713,2.85972,3.44639,6.11530,10.2591,17.1658,30.9949,54.4567,
             ...
             (1,150), 1.627),       #invisible, too weak compared to N2
    
    # A3+C3+c1 states 	A³Σu⁺/A'³Δu/c¹Σu⁻ <- ground (X3), unresolved triplet
    _Channel([7.52587,9.18439,11.1265,20.0687,34.2123,53.8234,87.0564,152.826,283.216,496.862],
             ...
             (7.6,496), 4.262),     #invisible, too weak
]

#Reaction O2 + e- -> O2+ + 2e- (ionization)
_O2P_CHANNELS = [
    # O2+ ground state (X2) O2⁺ X²Πg
    _Channel([16.2285,31.3318,52.1857,79.7560,107.446,136.806,167.823],
             ...
             (16.3,167), 12.072),       #invisible
    # O2+ B4 state (production from ground; emission observed B4->A4 only) O2⁺ b⁴Σg⁻→a⁴Πu
    _Channel([18.0957,22.3092,30.5386,43.2876,81.1131,168.761,389.860,1149.76,2848.04],
             ...
             (18.1,2800), 18.171),      # weak orange-red emission, not observed in aurora
]
...
def cross_section_inelastic_O2(eV):
    ...
    col_o2    : (n_ch1,) emission color [uint32] for each O2 excitation channel
    col_o2p   : (n_ch2,) emission color [uint32] for each O2+ production channel
    col_o2c   : (n_ch3,) emission color [uint32] for each O2- capture channel
    """
    ...
    col_o2 = np.array([channel.color_uint for channel in _O2_CHANNELS])
    col_o2p = np.array([channel.color_uint for channel in _O2P_CHANNELS])
    col_o2c = np.array([np.nan]) # not implemented

    return Qex_O2, Qex_O2p, Qex_O2c, loss_o2, loss_o2p, loss_o2c, col_o2, col_o2p, col_o2c
